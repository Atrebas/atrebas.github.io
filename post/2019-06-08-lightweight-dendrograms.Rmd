...
[This stackoverflow question](https://stackoverflow.com/questions/21474388/colorize-clusters-in-dendogram-with-ggplot2) provided me with some leads and I finally ended up writing the functions described below, using ggdendro and ggplot2 only.   
...
tanglegram <- function(hcdata1, hcdata2, cols){
  ...
  idx <- data.table(y1 = 1:nrow(hcdata1$labels),
...
tanglegram(hcdata1, hcdata2, cols)
```
The [gclus](https://cran.r-project.org/web/packages/gclus/) package can be used to reorder the hclust objects, facilitating the comparison.       

```{r, fig.height = 7, fig.width = 13, warning = FALSE, message = FALSE}
library(gclus)

hcdata3 <- dendro_data_k(reorder(hc1, dis = D), 5)
hcdata4 <- dendro_data_k(reorder(hc2, dis = D), 5)

tanglegram(hcdata3, hcdata4, cols)
```
