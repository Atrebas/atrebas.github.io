...
- 針, needle  + 鼠, rat/mouse =  針鼠, hedgehog, pronounced _harinezumi_     
...
Kana are syllabic systems, each character corresponds to one sound (a mora). Hiragana and katakana represent the same sounds but with differents sets of characters. Like kanji, each kana has a given number of strokes and must be written in a precise order.    
...
Below is a table showing the kana characters with the romanized pronunciations ([source](https://upload.wikimedia.org/wikipedia/commons/a/a6/Hiragana-Katakana-Romanization.JPG)).
![kana-table](/images/hiragana-katakana-romanization.jpg)
...
But they are easy to understand once you get the essentials rules presented above.   
...
- The code that follows was used to produce the final [pdf](/files/kana.pdf) plots for both hiragana and katakana.    
...
kana   <- fread("kana3.csv", encoding = "UTF-8")
kana
